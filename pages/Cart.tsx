...
const Cart: React.FC = () => {
    ...
    const handleWhatsAppOrder = () => {
        ...
        cart.forEach((item, index) => {
            ...
            itemsList += `   ${t.whatsapp.category}: ${item.category}\n`;
            itemsList += `   ${t.whatsapp.unitPrice}: ${item.price.toFixed(2)} DH\n`;
            itemsList += `   ${t.whatsapp.quantity}: ${item.quantity}\n`;
            itemsList += `   ${t.whatsapp.subtotal}: ${(item.price * item.quantity).toFixed(2)} DH\n\n`;
        });

        const message = `🛍️ *${t.whatsapp.newOrder}*\n\n` +
            `📦 *${t.whatsapp.orderedProducts}:*\n\n${itemsList}` +
            `💵 *${t.cart.total.toUpperCase()}: ${cartTotal.toFixed(2)} DH*\n\n` +
            `${t.whatsapp.orderMessage}`;
    ...
    if (cart.length === 0) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center bg-secondary animate-fade-in">
                <h2 className="font-serif text-3xl text-primary mb-4">{t.cart.cartEmpty}</h2>
                <p className="text-primary/60 mb-8">{t.cart.cartEmptyDesc}</p>
    ...
    return (
        <div className="bg-secondary min-h-screen pt-12 pb-24 animate-fade-in">
            <div className="container mx-auto px-6 md:px-12">
                <h1 className="font-serif text-4xl text-primary mb-12 text-center">{t.cart.yourCart}</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white p-8 border border-primary/5 sticky top-24">
                            <h3 className="font-serif text-xl text-primary mb-6">{t.cart.orderSummary}</h3>
